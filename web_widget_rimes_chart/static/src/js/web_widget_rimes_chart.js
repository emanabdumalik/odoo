odoo.define('web_widget_rimes_chart', function (require) {
 ...
 var formats=require('web.formats');
    ...
    var RimesChart = widget.FieldChar.extend({
        template: "FieldRimesChart",
        
        widget_class: 'o_form_field_canvas',
        events: {
            'change input': 'store_dom_value',
            'click #vv':'init_go',
            'click button':'handleCommand',
...
this.idag=null;
    ...
    var $ = go.GraphObject.make;  // for conciseness in defining templates
      self.myDiagram =
        self.$$(go.Diagram, "myDiagramDiv",  // must name or refer to the DIV HTML element
          {
...
function enable(name, ok) {
  ...
  setTimeout(enableAll, 1000);
      ...
      self.myPalette =
        self.$$(go.Palette, "myPaletteDiv",  // must name or refer to the DIV HTML element
          {
...
self.initialized=true;            
        ...
        set_thumbnail:function(data){
    //   this.thumb= data.makeImageData()
        }, 
        load:function(){
        	var self=this;
          var show_value = self.format_value(self.get('value'), JSON.stringify(self.default));
        ...
        is_syntax_valid: function() {
            if (!this.get("effective_readonly") && this.$("input").size() > 0) {
                try {
                    this.parse_value(this.$('input').val(), JSON.stringify(self.default));
                    return true;
                } catch(e) {
                    return false;
                }
        ...
        render_value: function() {
          var self=this;
            var show_value = this.format_value(this.get('value'), JSON.stringify(self.default));
...
//}
//self.$('#img').attr('src',self.myDiagram.makeImageData({size: new go.Size(NaN,750)}))
...
//},5000)
    ...
    core.form_widget_registry.add('rimes_chart', RimesChart);
 return {
        RimesChart: RimesChart
    };
